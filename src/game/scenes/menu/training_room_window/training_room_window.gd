...
@export var health_stepper: TrainingStepper
@export var questions_stepper: TrainingStepper
@export var min_number_stepper: TrainingStepper
@export var max_number_stepper: TrainingStepper
@export var answer_speed_stepper: TrainingStepper
@export var early_exam_questions_stepper: TrainingStepper
@export var early_exam_speed_stepper: TrainingStepper
...
func _set_stepper_value(stepper: TrainingStepper, value: float) -> void:
	if stepper != null:
...
func _add_round_coeff_row(round_num: int, coeff: float) -> void:
	...
	var round_stepper: TrainingStepper = stepper_template.instantiate() as TrainingStepper
	...
	var coeff_stepper: TrainingStepper = stepper_template.instantiate() as TrainingStepper
...
func _sync_round_coeffs_from_ui() -> void:
	...
	for row in round_coeffs_box.get_children():
		if not row is HBoxContainer or row.get_child_count() < 4:
			continue
		var round_stepper: TrainingStepper = row.get_child(1)
		var coeff_stepper: TrainingStepper = row.get_child(3)
